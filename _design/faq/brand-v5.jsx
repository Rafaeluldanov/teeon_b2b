/* V5 brand — Teago-inspired stylized mark + wordmark + photo placeholder */

// Custom mark — bold "Т" / "T" with separated triangle accent (Teago vibe)
const MarkV5 = ({ size = 80, color = 'currentColor', accent = 'currentColor', style }) => (
  <svg width={size} height={size * 1.05} viewBox="0 0 100 105" fill="none" style={{ display: 'block', ...style }}>
    {/* Stem of T */}
    <path d="M 30 6 L 70 6 L 70 30 L 56 30 L 56 100 L 26 100 L 26 30 L 30 30 Z" fill={color} />
    {/* Separated triangle accent (top-right) */}
    <path d="M 76 6 L 100 6 L 100 32 Z" fill={accent} />
    {/* Inner notch suggesting movement */}
    <path d="M 56 30 L 56 56 L 76 30 Z" fill={accent} opacity="0.85" />
  </svg>
);

const Logotype = ({ size = 36, color = 'var(--ink)', mark = 'var(--ink)', accent = 'var(--yellow)' }) => (
  <span style={{ display: 'inline-flex', alignItems: 'center', gap: size * 0.34 }}>
    <MarkV5 size={size} color={mark} accent={accent} />
    <span style={{
...
const Photo = ({ tint = 'navy', label, chip, num, kind, style, children }) => {
  const grads = {
    navy:   'linear-gradient(160deg, #3a4250 0%, #0a0f1a 100%)',
    ...
    teal:   'linear-gradient(160deg, #5b8b9e 0%, #0e2630 100%)',
    cream:  'linear-gradient(160deg, #ECE3BE 0%, #b8a880 100%)',
  };
  return (
    <div style={{
      position: 'relative', width: '100%', height: '100%',
      background: grads[tint] || grads.navy,
...
Object.assign(window, { MarkV5, Logotype, Photo });
