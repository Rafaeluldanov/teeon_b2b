/* V3 — Brand-identity bento sections */

const { Mark, Logotype, Star, Photo } = window;

/* ---------- NAV ---------- */
const NavV3 = () => (
  <nav className="nav">
    <a href="#" className="nav__logo">
      <Logotype mark="var(--yellow)" bowl="var(--paper)" color="var(--paper)" />
    </a>
...
const HeroV3 = () => (
  <section className="hero-grid">
    {/* Main hero — yellow with massive wordmark */}
    <div className="h-main">
      <div>
        <div className="h-main__top">
          <span className="h-main__chip">
            <span className="badge">B2B</span>
            Корпоративный мерч с 2014
          </span>
          <span className="h-main__pill">
            <span className="dot"></span>
            Открыты для проектов · отгрузка 14 дней
          </span>
        </div>
      </div>

      <div className="wordmark">
        мерч<span className="dot">.</span>цех
      </div>

      <div>
        <div className="h-main__tagline">
          Шьём промо-одежду и корпоративный мерч в собственном цехе. От 30 шт., с 9 методами нанесения и образцом до тиража.
        </div>
        <div className="h-main__row" style={{ marginTop: 20 }}>
    ...
    <div className="h-grid">
      <div className="h-grid__lines"></div>
      <div className="h-grid__inner">
        <div>
          <div className="h-grid__label">Бренд-марка / 01</div>
          <div style={{ marginTop: 12, fontFamily: 'var(--display)', fontWeight: 800, fontSize: 22, letterSpacing: '-0.02em' }}>
            На клейке, бирке, упаковке.<br />Один знак — везде.
          </div>
        </div>
        <div className="h-grid__mark">
          <Mark size={260} color="var(--ink)" bowl="var(--blue)" />
        </div>
      </div>
    </div>

    {/* Strip of 4 small cards */}
    <div className="h-strip">
      <div className="h-card blue">
        <div>
          <div className="h-card__num">01 / Своё производство</div>
          <div className="h-card__title" style={{ marginTop: 14 }}>Один цех от лекала до отгрузки</div>
        </div>
        <div className="h-card__body">Пошив, нанесение, упаковка — без посредников и логистики между подрядчиками.</div>
      </div>

      <div className="h-card yellow">
        <div className="h-card__num">02 / Сроки</div>
        <div>
          <div className="h-card__big">10–14<span className="u">дней</span></div>
          <div className="h-card__body" style={{ marginTop: 8 }}>от макета до отгрузки на стандартном тираже</div>
        </div>
      </div>

      <div className="h-card ink">
        <div className="h-card__num" style={{ color: 'var(--yellow)' }}>03 / Методы нанесения</div>
        <div>
          <div className="h-card__big">9<span className="u">методов</span></div>
          <div className="h-card__body" style={{ marginTop: 8, opacity: .8 }}>от вышивки до DTF — под любую ткань и тираж</div>
        </div>
      </div>

      <div className="h-card paper" style={{ padding: 0 }}>
        <div style={{ position: 'relative', height: '100%', borderRadius: 'inherit', overflow: 'hidden' }}>
          <Photo tint="moss" kind="tee" label="БРЕНД·МЕРЧ" />
          <span style={{ position: 'absolute', top: 18, right: 18, background: 'var(--paper)', color: 'var(--ink)', padding: '8px 14px', borderRadius: 999, fontFamily: 'var(--body)', fontWeight: 700, fontSize: 12, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
            Фото · цех
          </span>
        </div>
      </div>
...
/* ---------- ADVANTAGES ---------- */
const advV3 = [
  { n: '01', t: 'Свой швейный\nцех',        d: '24 швеи, 600 м², полный цикл от раскроя до упаковки.',  bg: '' },
  { n: '02', t: 'Брендирование\nна площадке',  d: 'Цех нанесения соседствует с пошивом — минус 5 дней.',     bg: 'yellow' },
  { n: '03', t: 'Образец\nдо тиража',       d: 'Pre-production sample, замена брака за наш счёт.',         bg: 'blue' },
  { n: '04', t: 'Документы\nи доставка',    d: 'УПД, договор, маркировка для WB/Ozon, ТК на выбор.',      bg: '' },
];

const AdvIco = ({ n }) => {
  const p = { width: 24, height: 24, fill: 'none', stroke: 'currentColor', strokeWidth: 1.8 };
  if (n === '01') return <svg {...p} viewBox="0 0 24 24"><path d="M5 10 L8 7 L10 8 Q12 9 14 8 L16 7 L19 10 L17 14 L15 13 L15 19 L9 19 L9 13 L7 14 Z"/></svg>;
  if (n === '02') return <svg {...p} viewBox="0 0 24 24"><circle cx="12" cy="12" r="7"/><path d="M8 12 L11 15 L16 9"/></svg>;
  if (n === '03') return <svg {...p} viewBox="0 0 24 24"><path d="M12 4 L19 7 L19 12 Q19 17 12 20 Q5 17 5 12 L5 7 Z"/><path d="M8 12 L11 15 L16 9"/></svg>;
  return <svg {...p} viewBox="0 0 24 24"><rect x="3" y="9" width="13" height="9"/><path d="M16 11 L20 11 L21 13 L21 17 L16 17"/><circle cx="8" cy="19" r="2"/><circle cx="19" cy="19" r="2"/></svg>;
};
...
const AboutV3 = () => (
  <section id="about">
    <div className="section-head">
      ...
      <p>Свой цех в Подмосковье — 24 швеи, своё нанесение, своя упаковка. Все этапы под одной крышей: лекало, раскрой, пошив, брендирование, отгрузка.</p>
    </div>

    <div className="about-grid">
      <div className="tile yellow col-5" style={{ minHeight: 380 }}>
        <div className="kicker">с 2014 года</div>
        <h2 style={{ marginTop: 16 }}>240<span style={{ color: 'var(--blue)' }}>+</span></h2>
        <h3 style={{ marginTop: 8, fontWeight: 600 }}>проектов в год</h3>
        ...
        <div style={{ marginTop: 'auto', display: 'flex', gap: 8, flexWrap: 'wrap', paddingTop: 24 }}>
          {['УПД', 'Договор', 'Отсрочка', 'Маркировка'].map(t => (
            <span key={t} style={{ padding: '8px 14px', background: 'rgba(10,14,26,.10)', borderRadius: 999, fontWeight: 600, fontSize: 12 }}>{t}</span>
          ))}
        </div>
      </div>
      ...
      <div className="col-3" style={{ display: 'grid', gap: 16 }}>
        <div className="tile ink">
          <div className="kicker" style={{ opacity: .6 }}>Площадь</div>
          <div className="big" style={{ marginTop: 12 }}>600<span style={{ fontSize: 28, color: 'var(--yellow)', marginLeft: 4 }}>м²</span></div>
          <div className="kicker" style={{ marginTop: 'auto', opacity: .6, color: 'var(--paper)' }}>производство + склад</div>
        </div>
        <div className="tile blue">
          <div className="kicker" style={{ opacity: .7 }}>Команда</div>
          <div className="big" style={{ marginTop: 12 }}>24<span style={{ fontSize: 28, color: 'var(--yellow)', marginLeft: 4 }}>швеи</span></div>
          <div className="kicker" style={{ marginTop: 'auto', opacity: .7, color: 'var(--paper)' }}>+ 6 чел. нанесение</div>
        </div>
      </div>

      <div className="tile col-6" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: 240 }}>
        <div className="kicker">за 11 лет</div>
        <div>
          <h2 style={{ fontSize: 64 }}>≈ 320 000 единиц</h2>
          <p style={{ marginTop: 12, color: 'var(--muted)' }}>пошито и отгружено клиентам · от 30-шт. пилотов до 5000-шт. сетевых партий</p>
        </div>
      </div>

      <div className="tile photo col-6" style={{ minHeight: 240 }}>
        <Photo tint="sky" kind="hoodie" num="02/" label="ГОТОВАЯ ПАРТИЯ" />
      </div>
    </div>
  </section>
);
...
/* ---------- CATALOG ---------- */
const catV3 = [
  { slug: 'futbolki',  num: '01', name: 'Футболки',  desc: 'Хлопок 160–220 г/м², от 30 шт.',      kind: 'tee',        bg: 'yellow' },
  { slug: 'hudi',      num: '02', name: 'Худи',      desc: 'Двунитка/трёхнитка с начёсом.',      kind: 'hoodie',     bg: 'blue', onDark: true },
  { slug: 'svitshoty', num: '03', name: 'Свитшоты',  desc: 'Без капюшона, плотность 280–340.',   kind: 'sweat',      bg: 'paper' },
  { slug: 'longslivy', num: '04', name: 'Лонгсливы', desc: 'Слим и oversize, длинный рукав.',     kind: 'longsleeve', bg: 'ink', onDark: true },
  { slug: 'sumki',     num: '05', name: 'Сумки',     desc: 'Шопперы и рюкзаки, бязь и оксфорд.',  kind: 'bag',        bg: 'paper' },
  { slug: 'zhiletki',  num: '06', name: 'Жилетки',   desc: 'Утеплённые и сетчатые, под печать.',  kind: 'vest',       bg: 'yellow' },
  { slug: 'kurtki',    num: '07', name: 'Куртки',    desc: 'Софтшелл, ветровки, утеплённые.',     kind: 'jacket',     bg: 'blue', onDark: true },
  { slug: 'dozhdeviki',num: '08', name: 'Дождевики', desc: 'PVC и нейлон, с капюшоном.',          kind: 'raincoat',   bg: 'paper' },
];
      ...
      {catV3.map(c => (
        <article className="cat" key={c.slug}>
          <div className={`cat__media ${c.bg}`}>
            <span className={`cat__num ${c.onDark ? 'on-dark' : ''}`}>{c.num} /</span>
            <span className="cat__pill">брендирование</span>
            <svg viewBox="0 0 200 240" preserveAspectRatio="xMidYMid slice"
                 style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', opacity: 0.55 }}>
              {c.kind === 'tee'        && <g fill="currentColor"><path d="M52 78 L74 56 L86 64 Q100 76 114 64 L126 56 L148 78 L138 96 L122 88 L122 188 L78 188 L78 88 L62 96 Z"/></g>}
              {c.kind === 'hoodie'     && <g fill="currentColor"><path d="M70 70 Q70 50 100 50 Q130 50 130 70 L150 78 L142 100 L130 94 L130 188 L70 188 L70 94 L58 100 L50 78 Z"/></g>}
              {c.kind === 'sweat'      && <g fill="currentColor"><path d="M52 78 L78 58 Q100 68 122 58 L148 78 L140 100 L128 94 L128 188 L72 188 L72 94 L60 100 Z"/></g>}
              {c.kind === 'longsleeve' && <g fill="currentColor"><path d="M40 70 L78 58 Q100 68 122 58 L160 70 L158 130 L140 124 L140 188 L60 188 L60 124 L42 130 Z"/></g>}
              {c.kind === 'bag'        && <g fill="currentColor"><path d="M70 92 L130 92 L138 192 L62 192 Z"/><path d="M84 92 Q84 60 100 60 Q116 60 116 92" fill="none" stroke="currentColor" strokeWidth="6"/></g>}
              {c.kind === 'vest'       && <g fill="currentColor"><path d="M58 78 L86 60 L100 70 L114 60 L142 78 L142 188 L114 188 L114 96 L86 96 L86 188 L58 188 Z"/></g>}
              {c.kind === 'jacket'     && <g fill="currentColor"><path d="M50 78 L78 56 L98 60 L98 188 L60 188 Z"/><path d="M150 78 L122 56 L102 60 L102 188 L140 188 Z"/></g>}
              {c.kind === 'raincoat'   && <g fill="currentColor"><path d="M64 80 Q64 56 100 56 Q136 56 136 80 L154 88 L144 110 L132 104 L132 196 L68 196 L68 104 L56 110 L46 88 Z"/></g>}
            </svg>
          </div>
...
/* ---------- BRANDING ---------- */
const brV3 = [
  { slug: 'embroidery',  num: '01', t: 'Вышивка',     d: 'Объёмная фактура, износостойко, премиум.',     tags: ['до 12 цв.', 'от 30 шт'], bg: '' },
  { slug: 'silk',        num: '02', t: 'Шелкография', d: 'Стойкая печать большими тиражами.',            tags: ['до 8 цв.', 'от 50'],     bg: 'yellow' },
  { slug: 'dtf',         num: '03', t: 'DTF',         d: 'Полноцветная плёнка, фото и мелкие детали.',  tags: ['CMYK+W', 'от 1 шт'],     bg: 'blue' },
  { slug: 'dtg',         num: '04', t: 'DTG',         d: 'Прямая печать на ткани, фото-качество.',       tags: ['фото-кач.', 'от 1 шт'],  bg: '' },
  { slug: 'sublim',      num: '05', t: 'Сублимация',  d: 'Полноцвет по всей поверхности синтетики.',     tags: ['100% PE', 'all-over'],   bg: 'ink' },
  { slug: 'chevrons',    num: '06', t: 'Шевроны',     d: 'Нашивки с пришивкой или на липучке.',          tags: ['ПВХ/ткань', 'от 50'],    bg: '' },
  { slug: 'deboss',      num: '07', t: 'Тиснение',    d: 'Сухое и фольгированное по коже и ткани.',      tags: ['фольга', 'клише'],       bg: 'yellow' },
  { slug: 'laser',       num: '08', t: 'Гравировка',  d: 'Лазер по металлу, дереву, коже.',              tags: ['от 1 шт', 'без краски'], bg: '' },
  { slug: 'labels',      num: '09', t: 'Бирки',       d: 'Тканые, кожаные, бумажные — финал.',           tags: ['жаккард', 'крой'],       bg: 'blue' },
];

const BrIco = ({ slug }) => {
  const p = { width: 24, height: 24, fill: 'none', stroke: 'currentColor', strokeWidth: 1.8 };
  switch (slug) {
    case 'embroidery': return <svg {...p} viewBox="0 0 24 24"><circle cx="12" cy="12" r="7"/><path d="M12 5 L12 19 M5 12 L19 12"/></svg>;
    case 'chevrons':   return <svg {...p} viewBox="0 0 24 24"><path d="M3 16 L12 7 L21 16 L18 19 L12 13 L6 19 Z"/></svg>;
    case 'silk':       return <svg {...p} viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18"/><path d="M7 8 L17 8 M7 12 L17 12 M7 16 L17 16"/></svg>;
    case 'dtf':        return <svg {...p} viewBox="0 0 24 24"><rect x="3" y="5" width="18" height="14"/><path d="M8 5 L8 19 M13 5 L13 19 M18 5 L18 19"/></svg>;
    case 'dtg':        return <svg {...p} viewBox="0 0 24 24"><path d="M3 8 L21 8 L18 19 L6 19 Z"/><circle cx="12" cy="13" r="2.5"/></svg>;
    case 'sublim':     return <svg {...p} viewBox="0 0 24 24"><path d="M5 19 Q5 5 12 5 Q19 5 19 19 Z"/></svg>;
    case 'deboss':     return <svg {...p} viewBox="0 0 24 24"><rect x="3" y="5" width="18" height="14"/><path d="M8 12 L16 12" strokeWidth="3"/></svg>;
    case 'laser':      return <svg {...p} viewBox="0 0 24 24"><path d="M3 21 L21 21 M12 3 L12 17 M9 14 L12 17 L15 14"/></svg>;
    case 'labels':     return <svg {...p} viewBox="0 0 24 24"><path d="M3 7 L15 3 L21 12 L15 21 L3 18 Z"/><circle cx="9" cy="12" r="2"/></svg>;
    default: return null;
  }
};
  ...
  <section id="how-we-work">
    <div className="section-head">
      <div>
        ...
        <h2>Шесть шагов <em>до отгрузки</em></h2>
      </div>
    ...
    <div className="steps-grid">
      {stV3.map(s => (
        <div className={`step ${s.active ? 'active' : ''}`} key={s.n}>
          <div className="step__num">{s.n.charAt(0)}<em>{s.n.charAt(1)}</em></div>
...
/* ---------- PORTFOLIO ---------- */
const ptV3 = [
  { slug: 'it-startup',  type: 'IT-компания',     t: 'Welcome-кит для онбординга',         task: 'Welcome-набор', ed: '320 шт',   tech: 'Вышивка + DTF',  term: '12 дней', date: '14.05.26', kind: 'hoodie',     bg: 'yellow' },
  { slug: 'event',       type: 'Event-агентство', t: 'Форма для open-air фестиваля',       task: 'Худи и кепки',  ed: '180 шт',   tech: 'Шелкография',    term: '9 дней',  date: '02.05.26', kind: 'cap',        bg: 'blue' },
  { slug: 'sport',       type: 'Спорт-клуб',      t: 'Форма для любительской лиги',        task: 'Игровая',       ed: '90 комп.', tech: 'Сублимация',     term: '14 дней', date: '21.04.26', kind: 'tee',        bg: 'ink' },
  { slug: 'restaurant',  type: 'Ресторан',        t: 'Униформа зала с вышивкой логотипа',  task: 'Рубашки',       ed: '60 шт',    tech: 'Вышивка',        term: '11 дней', date: '12.04.26', kind: 'longsleeve', bg: 'paper' },
  ...
  { slug: 'coffee',      type: 'Кофейня',         t: 'Фартуки и футболки барист',          task: 'Униформа',      ed: '45 шт',    tech: 'Вышивка',        term: '8 дней',  date: '15.03.26', kind: 'vest',       bg: 'blue' },
];
        ...
        <article className="case" key={c.slug}>
          <div className={`case__media ${c.bg}`}>
            ...
            <svg viewBox="0 0 200 240" preserveAspectRatio="xMidYMid slice"
              ...
              {c.kind === 'bag'        && <g fill="currentColor"><path d="M70 92 L130 92 L138 192 L62 192 Z"/><path d="M84 92 Q84 60 100 60 Q116 60 116 92" fill="none" stroke="currentColor" strokeWidth="6"/></g>}
              {c.kind === 'vest'       && <g fill="currentColor"><path d="M58 78 L86 60 L100 70 L114 60 L142 78 L142 188 L114 188 L114 96 L86 96 L86 188 L58 188 Z"/></g>}
              {c.kind === 'cap'        && <g fill="currentColor"><path d="M60 130 Q60 80 100 80 Q140 80 140 130 L155 138 L155 150 L45 150 L45 138 Z"/></g>}
            </svg>
          </div>
          <div className="case__body">
            ...
            <div className="case__actions">
              ...
              <a href="#request" style={{ color: 'var(--blue)', fontWeight: 700, fontSize: 13, letterSpacing: '0.02em', textTransform: 'uppercase', whiteSpace: 'nowrap', marginLeft: 'auto' }}>
                Рассчитать похожий →
              </a>
            </div>
          </div>
...
const clV3 = [
  ...
  { mark: 'CF', n: 'Кофейня',             tp: 'Сеть, 12 точек',        bg: 'ink' },
  { mark: 'RT', n: 'Ритейл-сеть',         tp: 'Fashion · 30+ маг.',    bg: '' },
  { mark: 'MA', n: 'Маркетинг-агентство', tp: 'B2B-промо',             bg: 'yellow' },
];
...
const SeoV3 = () => (
  <section className="seo">
    <div>
      <div className="kicker" style={{ marginBottom: 18, color: 'var(--muted)', fontFamily: 'var(--body)', fontWeight: 600, fontSize: 12, letterSpacing: '0.14em', textTransform: 'uppercase' }}>
        (09) — Подробнее
      </div>
        ...
        <form className="form" onSubmit={e => { e.preventDefault(); setSent(true); }}>
          {sent ? (
            <div style={{ padding: '60px 0', textAlign: 'center' }}>
              <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'var(--yellow)', color: 'var(--ink)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', marginBottom: 22 }}>
                <svg width="34" height="34" viewBox="0 0 28 28" fill="none"><path d="M6 14 L12 20 L22 8" stroke="currentColor" strokeWidth="2.5"/></svg>
              </div>
              <div style={{ fontFamily: 'var(--display)', fontSize: 40, fontWeight: 800, letterSpacing: '-0.03em', marginBottom: 12 }}>
                Заявка принята
              </div>
              <p style={{ color: 'var(--muted)', maxWidth: 360, margin: '0 auto', fontSize: 15 }}>
      ...
      <div className="strip__track">
        {loop.map((t, i) => (
          <React.Fragment key={i}>
            ...
            <span className="star"><Star size={28} color="var(--yellow)" /></span>
          </React.Fragment>
        ))}
      </div>
    </div>
